...
def get_timetable_nhk(year, month, day, area):
	...
	for program_cell in program_cells:
		...
		# アイコン
		types = nhk_radio.extractIcons(program_cell)

		bar = create_diff(station_tags[idx], start_time, types, name_id, chapter_id, splited_by_space, desc_id)
...
def get_timetable(year,month,day,station):
	...
	for program_part in program_parts:
		...
		bar = create_diff(station_tag, start_time, types, name_id, chapter_id, splited_by_space, desc_id)
		programs.append(bar)
